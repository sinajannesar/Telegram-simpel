...
export default function Chat() {
    ...
    useEffect(() => {
        if (status === "authenticated" && (session === null || session === void 0 ? void 0 : session.accessToken)) {
    ...
    if (status === "unauthenticated") {
        return (<div className="flex flex-col items-center justify-center h-screen bg-gradient-to-br from-gray-900 to-black text-white p-8 text-center">
        <h1 className="text-3xl font-bold mb-4">Access Denied</h1>
        <p className="text-lg mb-6">Please log in to use the chat feature.</p>
        <a href="/authregister/login" className="px-6 py-3 font-semibold rounded-xl shadow-lg bg-gradient-to-r from-blue-700 to-purple-600 text-white hover:from-blue-600 hover:to-purple-500 transition-all duration-300 ease-in-out transform hover:scale-105">
          Log In
        </a>
      </div>);
    }
    return (<div className="flex items-center justify-center min-h-screen bg-gradient-to-br from-slate-900 via-purple-900 to-slate-900 p-4">
      <div className="relative w-full max-w-4xl bg-gray-800/30 dark:bg-black/50 backdrop-blur-2xl rounded-3xl shadow-2xl border border-purple-500/30 transition-all duration-500 hover:shadow-[0_0_45px_rgba(168,85,247,0.5)] flex flex-col" style={{ height: 'calc(100vh - 80px)', maxHeight: '800px' }}>
        ...
        <div className="p-3 px-5 text-xs md:text-sm text-gray-300 dark:text-gray-400 border-b border-purple-400/20 relative z-10">
          <div className="flex justify-between items-center mb-2">
            <span>User: <span className="font-semibold text-purple-300">{((_a = session === null || session === void 0 ? void 0 : session.user) === null || _a === void 0 ? void 0 : _a.name) || "Anonymous"}</span></span>
            <span>Online: <span className="font-semibold text-purple-300">{onlineUsers.length}</span></span>
          </div>
          
          {/* Online Users List */}
          {onlineUsers.length > 0 && (<div className="flex flex-wrap gap-2 mt-2">
              <span className="text-xs text-gray-400">Online users:</span>
              {onlineUsers.map((user) => {
                var _a, _b;
                return (<span key={user.userId} className={`px-2 py-1 rounded-full text-xs ${user.userId === ((_a = session === null || session === void 0 ? void 0 : session.user) === null || _a === void 0 ? void 0 : _a.id)
                        ? 'bg-purple-600/50 text-purple-200'
                        : 'bg-gray-600/50 text-gray-300'}`}>
                  {user.userName} {user.userId === ((_b = session === null || session === void 0 ? void 0 : session.user) === null || _b === void 0 ? void 0 : _b.id) && '(You)'}
                </span>);
            })}
            </div>)}

          {/* Connection Error */}
          {connectionError && (<div className="mt-2 text-red-400 text-xs">
        ...
        <div className="flex-grow p-4 md:p-6 space-y-4 overflow-y-auto relative z-10 scrollbar-thin scrollbar-thumb-purple-500/40 scrollbar-track-gray-700/20 scrollbar-thumb-rounded-full">
          {messages.length === 0 && (<p className="text-center text-gray-400 dark:text-gray-500 py-10">
              No messages yet. Be the first one to start the conversation!
            </p>)}
          
          {messages.map((msg, index) => {
            var _a;
            const isCurrentUser = msg.userId === ((_a = session === null || session === void 0 ? void 0 : session.user) === null || _a === void 0 ? void 0 : _a.id);
            return (<div key={index} className={`flex ${isCurrentUser ? 'justify-end' : 'justify-start'}`}>
                <div className={`max-w-[70%] md:max-w-[60%] px-4 py-2.5 rounded-2xl shadow-md transition-all duration-300 ${isCurrentUser
                  ...
                  {!isCurrentUser && (<p className="text-xs font-semibold text-purple-300 dark:text-purple-400 mb-1">
                      {msg.userName}
                    </p>)}
